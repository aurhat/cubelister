...
void CslListCtrlPlayerSearch::AddResult(CslServerInfo *info, CslPlayerStatsData *player, bool rebuild)
{
    ...
    ListSetItem(i, COLUMN_PLAYER, player->Name, GetCountryFlag(::wxGetApp().GetCslEngine(), player->IP));
    ListSetItem(i, COLUMN_SERVER, info->GetBestDescription(), GetGameImage(::wxGetApp().GetCslEngine(), info->GetGame().GetFourCC()));
...
void CslListCtrlPlayerSearch::RemoveServer(CslServerInfo *info)
{
    loopv(m_entries)
    {
        CslPlayerSearchEntry *entry=(CslPlayerSearchEntry*)m_entries[i];
